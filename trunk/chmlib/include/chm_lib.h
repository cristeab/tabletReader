...
#else
FUN_PREFIX struct chmFile* chm_open(const char *filename);
#endif

/* close an ITS archive */
FUN_PREFIX void chm_close(struct chmFile *h);

/* methods for ssetting tuning parameters for particular file */
#define CHM_PARAM_MAX_BLOCKS_CACHED 0
FUN_PREFIX void chm_set_param(struct chmFile *h,
                   int paramType,
                   int paramVal);
...
FUN_PREFIX int chm_resolve_object(struct chmFile *h,
                       const char *objPath,
                       struct chmUnitInfo *ui);

/* retrieve part of an object from the archive */
FUN_PREFIX LONGINT64 chm_retrieve_object(struct chmFile *h,
                              struct chmUnitInfo *ui,
                              unsigned char *buf,
                              LONGUINT64 addr,
                              LONGINT64 len);
...
FUN_PREFIX int chm_enumerate(struct chmFile *h,
                  int what,
                  CHM_ENUMERATOR e,
                  void *context);

FUN_PREFIX int chm_enumerate_dir(struct chmFile *h,
                      const char *prefix,
                      int what,
                      CHM_ENUMERATOR e,
                      void *context);
